...
class CNN():
    ...
    def __init__(self, word_embeddings, setting):
        ...
        pos1_embedding = tf.get_variable(
            'pos1_embedding', [self.pos_num, self.pos_size])
        pos2_embedding = tf.get_variable(
            'pos2_embedding', [self.pos_num, self.pos_size])
        #relation_embedding = tf.get_variable('relation_embedding', [self.num_classes, self.cnn_size])

        self.input_word = tf.placeholder(
            dtype=tf.int32, shape=[None, len_sentence], name='input_word')
        self.input_pos1 = tf.placeholder(
            dtype=tf.int32, shape=[None, len_sentence], name='input_pos1')
        self.input_pos2 = tf.placeholder(
            dtype=tf.int32, shape=[None, len_sentence], name='input_pos2')
        self.input_y = tf.placeholder(dtype=tf.float32, shape=[
                                      None, num_classes], name='input_y')
        self.keep_prob = tf.placeholder(tf.float32)

        self.input_word_ebd = tf.nn.embedding_lookup(
            word_embedding, self.input_word)
        self.input_pos1_ebd = tf.nn.embedding_lookup(
            pos1_embedding, self.input_pos1)
        self.input_pos2_ebd = tf.nn.embedding_lookup(
            pos2_embedding, self.input_pos2)

        self.inputs = tf.concat(
            axis=2, values=[self.input_word_ebd, self.input_pos1_ebd, self.input_pos2_ebd])
        self.inputs = tf.reshape(
            self.inputs, [-1, self.len_sentence, self.word_embedding+self.pos_size*2, 1])

        conv = layers.conv2d(inputs=self.inputs, num_outputs=self.cnn_size, kernel_size=[
                             3, 60], stride=[1, 60], padding='SAME')
        ...
        self.outputs = layers.fully_connected(
            inputs=drop, num_outputs=self.num_classes, activation_fn=tf.nn.softmax)
        ...
        self.cross_loss = - \
            tf.reduce_mean(tf.reduce_sum(
                self.input_y * tf.log(self.outputs), axis=1))
        self.reward = tf.log(tf.reduce_sum(
            self.input_y * self.outputs, axis=1))
        ...
        self.accuracy = tf.reduce_mean(
            tf.cast(tf.equal(self.pred, self.y_label), 'float'))
            ...
            for epoch in range(1, settings.num_epochs+1):
                ...
                for _ in bar:
                    ...
                    _, loss, accuracy = sess.run(
                        [model.train_op, model.final_loss, model.accuracy], feed_dict=feed_dict)
                    bar.set_description(
                        'epoch {} loss={:.6f} accuracy={:.6f}'.format(epoch, loss, accuracy))
                    # break
...
class interaction():
    ...
    def __init__(self, sess, save_path='model/model.ckpt3'):
        ...
        with tf.variable_scope("model"):
            self.model = CNN(word_embeddings=wordembedding,
    ...
    def test(self, batch_test_word, batch_test_pos1, batch_test_pos2):
        ...
        feed_dict[self.model.input_pos2] = batch_test_pos2
        feed_dict[self.model.keep_prob] = 1
        relation, prob = self.sess.run(
            [self.model.pred, self.model.pred_prob], feed_dict=feed_dict)
    ...
    def update_cnn(self, update_word, update_pos1, update_pos2, update_y, updaterate):
        ...
        with self.sess.as_default():
            ...
            for i in tqdm(range(num_steps)):
                ...
                batch_word = update_word[i *
                                         self.settings.batch_size:(i+1)*self.settings.batch_size]
                batch_pos1 = update_pos1[i *
                                         self.settings.batch_size:(i+1)*self.settings.batch_size]
                batch_pos2 = update_pos2[i *
                                         self.settings.batch_size:(i+1)*self.settings.batch_size]
                batch_y = update_y[i *
                                   self.settings.batch_size:(i+1)*self.settings.batch_size]
            ...
            feed_dict = dictionary = dict(
                zip(self.model.tvars_holders, tvars_update))
    ...
    def produce_new_embedding(self):
        ...
        with self.sess.as_default():
            ...
            for batch in tqdm(range(len_batch)):
                ...
                tmp_sentence_ebd = self.sentence_ebd(
                    batch_word, batch_pos1, batch_pos2, batch_y)
...
if __name__ == '__main__':
    ...
    # train model
    print('train model')
    train('cnndata/cnn_train_word.npy', 'cnndata/cnn_train_pos1.npy',
          'cnndata/cnn_train_pos2.npy', 'cnndata/cnn_train_y.npy', 'model/origin_cnn_model.ckpt')
