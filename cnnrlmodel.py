...
class environment():
    ...
    def reset(self, e1, e2, batch_sentence_ebd, batch_reward):
        ...
        self.vector_mean = np.array(
            [0.0 for x in range(self.sentence_len)], dtype=np.float32)
        self.vector_sum = np.array(
            [0.0 for x in range(self.sentence_len)], dtype=np.float32)

        current_state = [self.vector_current,
                         self.vector_mean, self.id_e1, self.id_e2]
...
class agent():
    def __init__(self, lr, entity_ebd, s_size):
        ...
        entity_embedding = tf.get_variable(
            name='entity_embedding', initializer=entity_ebd, trainable=False)

        self.state_in = tf.placeholder(shape=[None, s_size], dtype=tf.float32)
        self.entity1 = tf.placeholder(
            dtype=tf.int32, shape=[None], name='entity1')
        self.entity2 = tf.placeholder(
            dtype=tf.int32, shape=[None], name='entity2')

        self.entity1_ebd = tf.nn.embedding_lookup(
            entity_embedding, self.entity1)
        self.entity2_ebd = tf.nn.embedding_lookup(
            entity_embedding, self.entity2)

        self.input = tf.concat(
            axis=1, values=[self.state_in, self.entity1_ebd, self.entity2_ebd])

        self.prob = tf.reshape(layers.fully_connected(
            self.input, 1, tf.nn.sigmoid), [-1])
        ...
        self.pi = self.action_holder * self.prob + \
            (1 - self.action_holder) * (1 - self.prob)
        ...
        self.update_batch = optimizer.apply_gradients(
            zip(self.gradient_holders, self.tvars))
...
def train():
    ...
    train_entitypair = np.load(
        './data/train_entitypair.npy', allow_pickle=True)
    y_train = np.load('data/train_y.npy', allow_pickle=True)

    all_sentence_ebd = np.load(
        './data/all_sentence_ebd.npy', allow_pickle=True)
    all_reward = np.load('./data/all_reward.npy', allow_pickle=True)
    average_reward = np.load('data/average_reward.npy', allow_pickle=True)
    ...
    with g_cnn.as_default():
        with sess1.as_default():
            interact = cnnmodel.interaction(
                sess1, save_path='model/origin_cnn_model.ckpt')
            ...
            for epoch in range(num_epoch):
                ...
                for batch in tqdm.tqdm(all_list):
                    ...
                    for j in range(sampletimes):
                        # reset environment
                        state = env.reset(batch_en1, batch_en2,
                                          batch_sentence_ebd, batch_reward)
                        list_action = []
                        list_state = []
                        old_prob = []
                    ...
                    for j in range(sampletimes):
                        ...
                        list_state_in = [
                            np.append(state[0], state[1]) for state in list_state]
                    ...
                    for i in range(batch_len):
                        ...
                        state = env.step(action)
                    chosen_reward = [batch_reward[x]
                                     for x in env.list_selected]
                ...
                interact.update_cnn(update_word, update_pos1,
                                    update_pos2, update_y, updaterate)
                print('finished')
                ...
                for index, var in enumerate(tvars_update):
                    tvars_update[index] = updaterate * \
                        tvars_new[index] + (1-updaterate) * tvars_old[index]
