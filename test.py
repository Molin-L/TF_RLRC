...
def produce_label_data():
    ...
    # logger.info('num_entitypair：'+str(num_entitypair))
    # logger.info('num_entitypair_true:'+str(num_entitypair_true))
...
def produce_pred_data(save_path, output_path):
    ...
    for step in tqdm(range(steps)):
        ...
        Evaluation
        """
        batch_relation, batch_prob = interact.test(
            batch_word, batch_pos1, batch_pos2)
        assert(len(batch_relation) == len(batch_prob))
...
def PR_curve(label_path, pred_path, num_total):
    ...
    with open(pred_path, 'rb') as input:
        pred_entitypair = pickle.load(input)
    print("Len of pred_entitypair:"+str(len(pred_entitypair)))
    list_pred = []
    for key in pred_entitypair.keys():
        tmp_prob = pred_entitypair[key][0]
        tmp_relation = pred_entitypair[key][1]
    ...
    for i, item in enumerate(list_pred):
        ...
        if tmp_relation in label_relations:
            true_positive += 1
        if i % (len(list_pred)//100) == 0 and i != 0:
            Precision.append(true_positive / i)
            Recall.append(true_positive/len(list_pred))
    print(true_positive)
...
if __name__ == '__main__':
    ...
    '''P@N'''
    produce_pred_data(save_path='model/best_cnn_model.ckpt',
                      output_path='result/best_pred_entitypair.pkl')
    result = P_N(label_path='data/label_entitypair.pkl',
                 pred_path='result/best_pred_entitypair.pkl')
    logger.info('best_cnn_P@100,200,300:'+' '.join([str(i) for i in result]))
    #[0.8, 0.735, 0.7066666666666667]

    produce_pred_data(save_path='model/origin_cnn_model.ckpt',
                      output_path='result/origin_pred_entitypair.pkl')
    result = P_N(label_path='data/label_entitypair.pkl',
                 pred_path='result/origin_pred_entitypair.pkl')
    logger.info('origin_cnn_P@100,200,300:'+' '.join([str(i) for i in result]))
